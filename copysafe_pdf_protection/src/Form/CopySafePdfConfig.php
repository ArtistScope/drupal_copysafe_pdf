...
/**
 * Configure copy safe pdf configuration for this site.
 *
 * @internal
 */
class CopySafePdfConfig extends ConfigFormBase {
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    $upload_directory = !empty($settings->get('uploadfolder')) ? $settings->get('uploadfolder') : 'upload_folder/copysafe_pdf_protection';
    ...
    foreach ($result as $record) {
      if (strpos($record->uri, $upload_location) !== FALSE) {
        ...
        $delete_url = Url::fromRoute('copysafe_pdf_protection.copysafe_pdf_file_delete', ['fid' => $record->fid])->toString();
        $del_link_html = new FormattableMarkup('<a href="@dellinkpath" class="button">@name</a>', ['@dellinkpath' => $delete_url, '@name' => 'Delete']);
        $embed_url = Url::fromRoute('copysafe_pdf_protection.open_embed_options_form', ['fid' => $record->fid])->toString();
